 ...
 * @param {string} commitBasePosition
 * @return {Promise.<string>} Downloaded file path
 */
function downloadChrome(osTarget, version, commitBasePosition) {
  return new Promise((resolve, reject) => {
    let chromeZipName = null;

    switch (osTarget) {
      case 'Mac':
        chromeZipName = 'chrome-mac.zip';
        break;

      case 'Linux_x32':
      case 'Linux_x64':
        chromeZipName = 'chrome-linux.zip';
        break;

      case 'Win_x32':
      case 'Win_x64':
        chromeZipName = 'chrome-win32.zip';
        break;
    }

    let dest = path.resolve(path.join(Constants.TEMP_DIRECTORY_PATH, `${version}.zip`));
    let chromeEndpoint = `https://www.googleapis.com/download/storage/v1/b/chromium-browser-snapshots/o/${osTarget}%2F${commitBasePosition}%2F${chromeZipName}?alt=media`;

    Logger.info('Download on the endpoint %s', chromeEndpoint);
    ...
    try {
      ...
      let readStream = request.get(chromeEndpoint);
...
class ChromeDownloader {
   ...
   * @return {Promise.<any>}
   */
  static downloadVersion(osTarget, version, commitBasePosition) {
    Logger.info('Start to download the expected version %s', version);

    return downloadChrome(osTarget, version, commitBasePosition)
